...
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog"; // Import dialog dari UI
  ...
  return (
    ...
    <Dialog open={isOpen} onOpenChange={onConfirm}>
      <DialogContent
        ...
        hideCloseButton // Properti khusus untuk menyembunyikan ikon "X" (jika diimplementasikan di komponen DialogContent)
      >
        <DialogHeader>
          <DialogTitle>
            <VisuallyHidden>{title}</VisuallyHidden> {/* Judul tetap tersedia untuk screen reader, tapi tidak tampil di layar */}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="flex flex-col items-center w-[380px] gap-3">
          ...
          />
          <div>
            <h2 className="text-[16px] font-medium">{title}</h2> {/* Judul visual */}
            <p className="text-sm text-gray-600">{message}</p> {/* Pesan deskriptif */}
          </div>
        </div>
